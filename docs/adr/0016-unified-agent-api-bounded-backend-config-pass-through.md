# ADR-0016 — Unified Agent API bounded backend config pass-through (`agent_api.config.*` + `backend.*`)
#
# Note: Run `make adr-fix ADR=docs/adr/0016-unified-agent-api-bounded-backend-config-pass-through.md`
...
This ADR defines how the Unified Agent API should expose *configuration/flag surfaces* from
...
- When we need to express “the host provides isolation externally”, use an explicit, dangerous core
  ...
  `docs/adr/0019-unified-agent-api-external-sandbox-exec-policy.md`) rather than ad-hoc backend
...
- `agent_api.exec.external_sandbox.v1` (boolean; explicit dangerous mode for externally sandboxed hosts;
  see `docs/adr/0019-unified-agent-api-external-sandbox-exec-policy.md`)
...
- **Codex**: has a generic `--config key=value` and generic `--enable/--disable <feature>` toggles.
  ...
  Unified Agent API cannot validate the effective configuration (keys are an open set).
...
If we allow arbitrary CLI arg pass-through, we lose the core Unified Agent API properties:
...
- Replace “raw string pass-through” with bounded, versioned backend keys when backend-specific knobs
  are still required.
- Keep safe defaults for general Unified Agent API consumers, while allowing internally sandboxed
...
- Supporting Claude Code `--plugin-dir` via the Unified Agent API (intentionally unbounded and
...
`docs/specs/unified-agent-api/extensions-spec.md` before shipping; backend keys must be defined in
...
Pros:
- Preserves the Unified Agent API “fail-closed, capability-gated” model while still allowing
...
- Define + implement:
  ...
  - `agent_api.exec.external_sandbox.v1` (uaa-0016; see
    `docs/adr/0019-unified-agent-api-external-sandbox-exec-policy.md`; needs owner-doc spec entry)
