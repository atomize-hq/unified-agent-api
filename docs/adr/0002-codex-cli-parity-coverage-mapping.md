...
- Discovery does not continue with the subset that enabled. Nothing downstream reads the `features` metadata, so a union built from a subset would look complete while feature-gated surfaces were missing. In `parity-acquire`, the failed leg uploads whatever raw help it captured before the failure as evidence (none when the root invocation is rejected), but no per-target snapshot. A failed non-required target leaves the acquisition incomplete (maintenance audit exit 4); a failed required target stops the union.
