...
- prompt sha256: `b2a9e591a8474d6edc8e6b1d7f0eb495071ae9ef9534c1f6b705ca892c937ed9`
...
- reopen pr branch: `automation/claude_code-maintenance-2.1.143`
- notes:
...
```md
# Packet PR Maintenance Prompt (`2.1.143`)
...
Execute the automated maintenance packet for `claude_code` target `2.1.143`.
...
1. Compare the current validated baseline from `cli_manifests/claude_code/latest_validated.txt` against the target `2.1.143` artifacts.
...
4. Refresh or create version-scoped manifest artifacts under `cli_manifests/claude_code/snapshots/2.1.143/`, `cli_manifests/claude_code/reports/2.1.143/`, and `cli_manifests/claude_code/versions/2.1.143.json` as required by the packet.
