...
cat > docs/agents/lifecycle/codex-maintenance/governance/automation-stand-down/0.157.0.toml <<'TOML'
schema_version = 1
agent_id = "codex"
target_version = "0.157.0"
reason = "closeout in progress"
request_recorded_at = "2026-09-26T08:36:16Z"
TOML
git add docs/agents/lifecycle/codex-maintenance/governance/automation-stand-down/0.157.0.toml
git commit docs/agents/lifecycle/codex-maintenance/governance/automation-stand-down/0.157.0.toml -m "chore(codex): stand automation down for 0.157.0"
...
```sh
cargo run -p xtask -- maintenance-stand-down-check --agent codex --target-version 0.157.0 --from-ref origin/staging
```

Nothing releases the freeze but retirement. Closing this PR, pushing to it, approving it and
merging it all leave it in force; the promotion PR for `0.157.0` removes the marker.
...
- branch linkage: `automation/codex-maintenance-0.157.0`
...
- reopen pr branch: `automation/codex-maintenance-0.157.0`
- notes:
