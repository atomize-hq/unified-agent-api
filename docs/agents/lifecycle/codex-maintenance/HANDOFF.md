...
cat > docs/agents/lifecycle/codex-maintenance/governance/automation-stand-down/0.156.0.toml <<'TOML'
schema_version = 1
agent_id = "codex"
target_version = "0.156.0"
reason = "closeout in progress"
request_recorded_at = "2026-09-24T08:27:43Z"
TOML
git add docs/agents/lifecycle/codex-maintenance/governance/automation-stand-down/0.156.0.toml
git commit docs/agents/lifecycle/codex-maintenance/governance/automation-stand-down/0.156.0.toml -m "chore(codex): stand automation down for 0.156.0"
...
```sh
cargo run -p xtask -- maintenance-stand-down-check --agent codex --target-version 0.156.0 --from-ref origin/staging
```

Nothing releases the freeze but retirement. Closing this PR, pushing to it, approving it and
merging it all leave it in force; the promotion PR for `0.156.0` removes the marker.
...
- prompt sha256: `04ae7fd12aa697301409574a8a4d092d48ebcf927f937123f0e4da94c51f311e`
...
- branch linkage: `automation/codex-maintenance-0.156.0`
...
- `cli_manifests/codex/snapshots/0.156.0/**`
- `cli_manifests/codex/reports/0.156.0/**`
- `cli_manifests/codex/versions/0.156.0.json`
...
- reopen pr branch: `automation/codex-maintenance-0.156.0`
- notes:
...
```md
# Packet PR Maintenance Prompt (`0.156.0`)
...
Execute the automated maintenance packet for `codex` target `0.156.0`.
...
1. Compare the current validated baseline from `cli_manifests/codex/latest_validated.txt` against the target `0.156.0` artifacts.
...
4. Refresh or create version-scoped manifest artifacts under `cli_manifests/codex/snapshots/0.156.0/`, `cli_manifests/codex/reports/0.156.0/`, and `cli_manifests/codex/versions/0.156.0.json` as required by the packet.
