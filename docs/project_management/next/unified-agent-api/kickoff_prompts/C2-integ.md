# Kickoff Prompt — C2-integ (Claude Code backend adapter)
...
5. Create the integration branch and worktree: `git worktree add -b uaa-c2-claude-integ wt/uaa-c2-claude-integ feat/unified-agent-api`.
...
4. Fast-forward merge `uaa-c2-claude-integ` into `feat/unified-agent-api`; set `C2-integ` to `completed`; add END entry to `session_log.md`; commit docs (`docs: finish C2-integ`).
