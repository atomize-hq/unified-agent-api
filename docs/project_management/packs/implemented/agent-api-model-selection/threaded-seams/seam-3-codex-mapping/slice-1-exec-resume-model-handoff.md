...
- **User/system value**: makes model selection work for Codex exec/resume flows via the existing builder path, with deterministic ordering and without any new raw parsing.
- **Scope (in/out)**:
  - In:
...
- **Acceptance criteria**:
  ...
  - `None` emits no `--model`
  - ordering follows `docs/specs/codex-streaming-exec-contract.md` (wrapper overrides before `--model`; capability-guarded `--add-dir` after)
