# Spec — Unified Agent API Extensions (Core keys + ownership rules)
...
- Built-in backends MUST NOT advertise this capability by default; it is intended for explicitly
  ...
  `docs/specs/unified-agent-api/contract.md`, "Dangerous capability opt-in (external sandbox exec policy)").
...
- Built-in backends MUST NOT treat this key, by itself, as authorizing additional backend-specific
  ...
  - Codex: MUST NOT rely on this key as authorization for additional Unified Agent API semantics
...
2) Update the Unified Agent API planning pack `spec_manifest.md` coverage matrix to assign the
