...
Usage: scripts/run-codex-worker.sh --worktree DIR --packet FILE \
  [--sandbox read-only|workspace-write] [--model NAME] [--reasoning-effort LEVEL] \
  [--profile NAME] [--timeout SECONDS] [--output-last-message FILE] [--dry-run]
...
--model and --reasoning-effort pin what the base config would otherwise decide.
The top level of ~/.codex/config.toml sets both, so omitting them does not mean
"no configuration" — it means whatever that file currently holds. Defaults here
are gpt-5.6-sol, and effort high for a write sandbox / xhigh for read-only.
EOF
}
...
# A review reads more carefully than an implementation writes, so read-only defaults higher.
if [[ -z "$reasoning_effort" ]]; then
  case "$sandbox" in read-only) reasoning_effort="xhigh" ;; *) reasoning_effort="high" ;; esac
fi
# An unrecognised value would reach Codex as a literal string and silently fall back, which
# would defeat the point of pinning it.
case "$reasoning_effort" in minimal|low|medium|high|xhigh) ;; *)
  printf 'error: --reasoning-effort must be minimal, low, medium, high or xhigh: %s\n' \
    "$reasoning_effort" >&2
  exit 2 ;;
esac
[[ -n "$model" ]] || { printf '%s\n' 'error: --model must not be empty' >&2; exit 2; }
...
cmd=(codex exec --sandbox "$sandbox" --cd "$worktree" --model "$model"
