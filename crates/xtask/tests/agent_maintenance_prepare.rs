...
fn prepare_agent_maintenance_write_creates_packet_root() {
    ...
    assert!(handoff.contains("cli_manifests/codex/latest_validated.txt"));
    assert!(handoff.contains("docs/specs/unified-agent-api/capability-matrix.md"));
    assert!(handoff.contains("docs/crates-io-release.md"));
    assert!(handoff.contains("this automated upstream-release lane is packet-only"));
