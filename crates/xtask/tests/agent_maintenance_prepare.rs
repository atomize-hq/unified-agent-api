...
fn prepare_agent_maintenance_write_creates_packet_root() {
    ...
    assert!(handoff.contains(".github/workflows/agent-maintenance-open-pr.yml"));
    assert!(handoff.contains("## Support-surface audit"));
    assert!(handoff.contains("pre-run debt count: `0`"));
    ...
    let threading =
        fs::read_to_string(fixture.join("docs/agents/lifecycle/codex-maintenance/threading.md"))
            .expect("read threading");
    assert!(threading.contains(
        "the actor handed the packet PR authors `docs/agents/lifecycle/codex-maintenance/governance/maintenance-closeout.json` and runs the exact `close-agent-maintenance` command"
    ));
    assert!(threading.contains(
        "An agent executing this packet inside a relay session must not run that command."
    ));
    ...
    assert!(pr_summary
        .contains("Execute the automated maintenance packet for `codex` target `0.98.0`."));
    ...
    assert!(pr_summary.contains("## Support-surface audit"));
...
#[test]
fn prepare_agent_maintenance_defaults_to_generic_open_pr_workflow_for_enrolled_agents() {
...
fn shared_renderer_keeps_handoff_pr_summary_and_prompt_in_lockstep() {
    ...
    let docs_with_guarded_commands = planned_docs
        .iter()
        .filter(|doc| {
            guarded_commands
                .iter()
                .any(|command| doc.contents.contains(command))
        })
        .collect::<Vec<_>>();
    assert_eq!(
        docs_with_guarded_commands
            .iter()
    ...
    // The list above is the load-bearing half: a new rendered doc that names guarded tooling
    // changes it and fails here, which is how `review_surfaces.md` was found. The check below is
    // document-level on purpose, and is therefore weaker than it reads — a doc carrying the phrase
    // once satisfies it even if some other section names a guarded command unframed, which is the
    // `## Recovery` defect `uaa-0068`'s second review round caught. Section-level proof would need a
    // heading parser and a per-doc exception table, because the prompt legitimately names guarded
    // commands in sections other than the one that frames them. See `uaa-0070`.
    for doc in docs_with_guarded_commands {
        assert!(
            doc.contents.contains("inside a relay session")
                || doc.contents.contains(
                    "Never invoke `execute-agent-maintenance`, `prepare-agent-maintenance`, or `refresh-agent`",
                ),
            "{} names guarded lifecycle tooling without relay framing",
            doc.relative_path
        );
    }
}
...
fn automated_request_with_execution_contract_toml() -> String {
    ...
    let prompt = contract_policy::packet_pr_prompt_template(
        entry,
