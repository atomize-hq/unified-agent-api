...
fn seed_opencode_packet_owned_contract_inputs(fixture: &std::path::Path) {
    ...
    write_text(
        &fixture.join(contract_policy::packet_owned_ops_playbook_path(maintenance_root)),
        "# Ops playbook\n",
    );
    write_text(
        &fixture.join(contract_policy::packet_owned_workflow_plan_path(maintenance_root)),
        "# CI workflows plan\n",
    );
    write_text(
        &fixture.join(contract_policy::packet_owned_prompt_template_path(maintenance_root)),
...
fn opencode_automated_request_toml(basis_ref: &str) -> String {
    automated_request_toml("opencode", basis_ref)
        ...
        .replace("source_ref = \"openai/codex\"", "source_ref = \"anomalyco/opencode\"")
        .replace(
...
fn automated_request_v2_with_detected_release_parses() {
    ...
    write_text(
        ...
        &opencode_automated_request_toml("docs/integrations/opencode/governance/seam-2-closeout.md"),
    );
...
fn automated_packet_refresh_renders_canonical_handoff_and_pr_summary() {
    ...
    assert!(handoff.contains("Execute the automated maintenance packet for `opencode` target `0.98.0`."));
    assert!(handoff.contains(
        "docs/agents/lifecycle/opencode-maintenance/governance/maintenance-request.toml"
    ));
    ...
    assert!(pr_summary.contains("Execute the automated maintenance packet for `opencode` target `0.98.0`."));
