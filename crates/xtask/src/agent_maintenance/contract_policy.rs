...
pub fn packet_pr_prompt_template(entry: &AgentRegistryEntry, maintenance_root: &str) -> String {
    format!(
        concat!(
            ...
            "3. For each `deferred_preexisting_gaps` row that also appears in `required_uplifts_this_run`, decide whether its `blocker_class` still holds at `{{{{VERSION}}}}`. If it does, re-authorize that existing row in `docs/specs/unified-agent-api/non-tui-support-debt.md` in place:\n",
            "   - set `authorized_at_version` to `{{{{VERSION}}}}`;\n",
            "   - set `scope_target_triples` to every target whose `cli_manifests/{agent_id}/reports/{{{{VERSION}}}}/coverage.<target>.json` lists the surface;\n",
