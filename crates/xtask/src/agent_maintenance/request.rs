    ...
    fn parse(value: &str, request_path: &Path) -> Result<Self, MaintenanceRequestError> {
        match value {
            ...
            other => Err(MaintenanceRequestError::Validation(format!(
                "maintenance request `{}` has invalid `trigger_kind` `{other}`; expected `drift_detected`, `manual_reopen`, or `post_release_audit`",
...
pub fn load_request(
    ...
    if raw.artifact_version != ARTIFACT_VERSION {
        return Err(MaintenanceRequestError::Validation(format!(
            "maintenance request `{}` has unsupported `artifact_version` `{}`; expected `{ARTIFACT_VERSION}`",
            relative_path.display(),
...
fn validate_repo_relative_reference(
