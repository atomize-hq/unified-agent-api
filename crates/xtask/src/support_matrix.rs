...
use publication::{validate_publication_artifacts, write_publication_artifacts};
...
pub const JSON_OUTPUT_PATH: &str = "cli_manifests/support_matrix/current.json";
pub const MARKDOWN_OUTPUT_PATH: &str = "docs/specs/unified-agent-api/support-matrix.md";
...
pub fn run(args: Args) -> Result<(), String> {
    ...
    let bundle = generate_publication_bundle(&workspace_root)?;
    if args.check {
...
pub fn generate_publication_bundle(workspace_root: &Path) -> Result<PublicationBundle, String> {
...
pub fn generate_publication_artifacts(workspace_root: &Path) -> Result<PublicationBundle, String> {
    ...
    let existing_markdown =
        fs::read_to_string(workspace_root.join(MARKDOWN_OUTPUT_PATH)).map_err(|err| {
            format!(
                "read {}: {err}",
