...
- Support publication separates:
  - manifest / upstream support
  - wrapper/backend-crate support
  - UAA unified support
- For a new agent, adding `cli_manifests/<agent>/` evidence does not by itself create backend or
  UAA support; backend support begins only once the agent-specific wrapper crate produces committed
  wrapper-derived evidence and reports.
...
- `supported` here is a wrapper-crate support statement. It is not, by itself, an `agent_api` or UAA unified-support claim.
